...
SonosUPnP::SonosUPnP(void (*ethernetErrCallback)(void))
{
...
void SonosUPnP::playUri(IPAddress speakerIP, const char *address)
{
...
bool SonosUPnP::upnpPost(IPAddress ip, uint8_t upnpMessageType, PGM_P action_P, const char *field, const char *valueA, const char *valueB, PGM_P extraStart_P, PGM_P extraEnd_P, const char *extraValue)
{
 ...
 {
  Serial.println("we didn't got a connection");
  return false;
  }
...
{
  if (strcmp(input, SONOS_STATE_ERROR_VALUE) == 0)              return SONOS_STATE_ERROR;
  if (strcmp(input, SONOS_STATE_STOPPED_VALUE) == 0)            return SONOS_STATE_STOPPED;
  ...
  if (strcmp(input, SONOS_STATE_TRANSITIONING_VALUE) == 0)      return SONOS_STATE_TRANSITIONING;
